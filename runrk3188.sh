...
./compile.sh BOARD=rk3188 COMPRESS_OUTPUTIMAGE=sha,gpg,xz DOWNLOAD_MIRROR="" EXTRAWIFI=no FORCE_CHECK_MD5_PACKAGES=no USE_TORRENT=no EXIT_PATCHING_ERROR=no BRANCH=current RELEASE=jammy BUILD_MINIMAL=no BUILD_DESKTOP=no BUILD_ONLY=default KERNEL_CONFIGURE=no DISABLE_IPV6=true CLEAN_LEVEL="make,make-kernel,make-uboot,debs,alldebs,images,cache,oldcache" BUILD_KSRC=yes INSTALL_HEADERS=yes
